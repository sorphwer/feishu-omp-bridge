...
async function handleStatus(_args: string, ctx: CommandContext): Promise<void> {
  ...
  const sess = ctx.sessions.latestSession(ctx.scope);
...
- phase: 模块阶段。常见值: ws(WebSocket), intake(消息入站), queue(去抖队列), flush(批处理), media(附件下载), prompt(prompt 组装), session(会话), agent(OMP 子进程), card(卡片渲染), cardAction(卡片回调), command(斜杠命令), sdk(飞书 SDK 内部)
...
async function handleDoctor(args: string, ctx: CommandContext): Promise<void> {
  ...
  // /doctor is admin-only and runs unrestricted, so it registers under `full`.
  const handle = ctx.activeRuns.register(ctx.scope, run, 'full');
...
async function submitConfig(ctx: CommandContext): Promise<void> {
  ...
  // `message_reply` no longer has a form control (the picker was hidden —
  // see config-card.ts), so `fv.message_reply` is normally absent. Fall back
  // to the CURRENT config value rather than a hardcoded default, so
  // submitting the form never silently flips a hand-set `messageReply` (e.g.
  // 'card') back to something else.
  const rawReply = String(fv.message_reply ?? '').trim();
  const messageReply: MessageReplyMode =
    rawReply === 'markdown' || rawReply === 'card'
