...
```mermaid
flowchart TD
  EV["事件 {senderId, chat, chatId}"] --> PRIN["principalOf → 组名 / guest"]
  PRIN --> RUN["run = principal.run (guest 恒 front)"]
  PRIN --> R{"首条命中 rule?"}
  R -->|"命中 (已知 profile)"| PROF["resolveProfile(rule.profile)"]
  R -->|"命中但 profile 名未知"| LK1["locked(name)"]
  R -->|无一命中| LK2["locked (ruleIndex -1)"]
  PROF --> OUT["{principal, run, profile, ruleIndex}"]
```
...
flowchart LR
  ...
  R -->|"否 (full)"| OV0{"discovery/memory 任一关?"}
  OV0 -->|否| EMPTY["返回空 (无 run 参数)"]
  OV0 -->|是| OVx["--config overlay"]
  R -->|"是 (受限)"| L1["① --tools 白名单<br/>builtinTools + cmd 名 + host 名"]
