...
```mermaid
flowchart TD
  FS["Feishu / Lark"] -->|"WebSocket / OpenAPI"| SDK["@larksuiteoapi/node-sdk · LarkChannel"]
  SDK -->|"normalized messages / card actions"| CH["src/bot/channel.ts"]
  CH -->|"AgentAdapter (唯一可替换的缝)"| OMP["src/agent/omp/OmpAdapter"]
  OMP -->|"JSONL stdio RPC"| PROC["omp --mode rpc<br/>--session-dir ~/.feishu-omp-bridge/omp-sessions"]
```
...
- [实现详解 `how-it-works/`](./how-it-works/) —— 逐子系统、逐文件、逐数据流的"事无巨细"实现说明（架构、消息管线、流式卡片、host 工具、访问策略、守护进程等，配 mermaid 图）。建议从 [索引](./how-it-works/README.md) 读起。
- [访问策略配置指南 `CONFIGURATION.zh.md`](./CONFIGURATION.zh.md) —— `policy`（principals / profiles / rules）完整配置参考与配方：按场景 × 身份选 profile、命令工具沙箱、自定义 `.mjs` hook、front/worker 分流。
...
flowchart LR
  DM1["你私聊"] --> FS["飞书"]
  DM2["别人私聊"] --> FS
  FS -->|"长连接"| FRONT["front (常开服务器, 唯一长连接)"]
  FRONT -->|"信任用户 → 中继"| WORKER["worker (你的笔记本)<br/>跑 omp · 同 app 凭据直接回飞书"]
  FRONT -.->|"其他人"| LOCAL["front 本地沙箱"]
```
